...
### 未编号公式 #92 — 未编号公式：\mathbf{P}^{-1} = \mathrm{...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.2. Conjugate gradient method for the primal approach，`explain.md` 第 655 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L655)
...
### 未编号公式 #93 — 未编号公式：\mathbf{x} = \mathbf{x}_k ...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.3. Schur complement for the dual approach，`explain.md` 第 665 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L665)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
6140:   }
```
### 未编号公式 #94 — 未编号公式：m_k(\mathbf{x}) = \Pi_k + ...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.3. Schur complement for the dual approach，`explain.md` 第 671 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L671)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
