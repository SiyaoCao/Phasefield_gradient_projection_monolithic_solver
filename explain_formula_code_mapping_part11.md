...
### 未编号公式 #56 — 未编号公式：\mathbf{W}_k = [\mathbf{Y}...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.2. Compact representation of limited-memory BFGS matrix，`explain.md` 第 420 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L420)
...
5729: 	  }
```
### 未编号公式 #57 — 未编号公式：\mathbf{M}_k = \begin{bmat...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.2. Compact representation of limited-memory BFGS matrix，`explain.md` 第 425 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L425)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
5729: 	  }
```
### 未编号公式 #58 — 未编号公式：\mathbf{D}_k = \mathrm{dia...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.2. Compact representation of limited-memory BFGS matrix，`explain.md` 第 431 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L431)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
6140:   }
```
### 未编号公式 #59 — 未编号公式：\mathbf{L}_k \in \mathbb{R...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.2. Compact representation of limited-memory BFGS matrix，`explain.md` 第 435 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L435)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
```
### Eq. (21) — B0 选取为对角分块切线
- 论文位置：[3. Gradient projection based monolithic scheme > 3.2. Compact representation of limited-memory BFGS matrix，`explain.md` 第 441 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L441)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
### 未编号公式 #61 — 未编号公式：t_i = \begin{cases} (x^0_i...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.3. Generalized Cauchy point，`explain.md` 第 455 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L455)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
