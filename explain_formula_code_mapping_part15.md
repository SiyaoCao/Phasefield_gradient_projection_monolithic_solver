...
### 未编号公式 #77 — 未编号公式：\mathcal{A}(\mathbf{x}^c) ...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization，`explain.md` 第 556 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L556)
...
```
### 未编号公式 #78 — 未编号公式：x_i = x^c_i = \begin{cases...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization，`explain.md` 第 561 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L561)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
```
### Eq. (25) — 子空间最小化目标
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization，`explain.md` 第 569 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L569)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
6140:   }
```
### 未编号公式 #80 — 未编号公式：x_i = x^c_i, \quad \forall...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization，`explain.md` 第 575 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L575)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
