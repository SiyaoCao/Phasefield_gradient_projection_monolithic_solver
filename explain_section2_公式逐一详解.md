# explain.md 第2章 公式逐一详解（完整推导与物理含义）
...
- \[ g(d) \]：相场退化函数。
- \[ k \]：小的非负残余刚度参数（理论上可取 0 或很小正数；文中算例取 \[k=0\]）。
...
其中 \[\psi_0\] 表示未进行张拉-受压分裂之前的经典线弹性应变能密度。
...
- \[\pmb N_{u_A}\]：结点 \[A\] 处位移向量形函数（粗体表示向量值形函数）。
- \[N_{d_A}\]：结点 \[A\] 处相场标量形函数（非粗体表示标量值形函数）。
- \[\pmb u_A,d_A\]：对应结点自由度。
...
- \[g'(d)\psi^+\] 对 \[d_B\] 导数给 \[g''(d)\psi^+N_{d_B}\]（此处 \[\psi^+\] 不直接显含 \[d\]，仅通过应变场参与耦合）；
- 梯度项导数给 \[(\nabla N_{d_A},g_c l\nabla N_{d_B})\]。
...
- 编号公式：\[(5)\]、\[(6)\]、\[(7)\]、\[(8)\]、\[(9)\]、\[(10)\]、\[(11)\]（分别在上文“公式(5)”至“公式(11)”小节中逐条展开）；
