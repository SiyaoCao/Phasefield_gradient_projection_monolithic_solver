...
### 未编号公式 #88 — 未编号公式：\hat{\mathbf{x}} = -\hat{\...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.1. Direct matrix factorization for the primal approach，`explain.md` 第 631 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L631)
...
```
### Eq. (29) — SMW 逆表达
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.1. Direct matrix factorization for the primal approach，`explain.md` 第 634 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L634)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
5272: 
```
### 未编号公式 #90 — 未编号公式：\mathbf{Z}^{\mathrm{T}}_k ...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.1. Direct matrix factorization for the primal approach，`explain.md` 第 639 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L639)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
5729: 	  }
```
### 未编号公式 #91 — 未编号公式：\mathbf{I} - \mathbf{M}_k ...
- 论文位置：[3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.1. Direct matrix factorization for the primal approach，`explain.md` 第 644 行附近](https://github.com/SiyaoCao/Phasefield_gradient_projection_monolithic_solver/blob/main/explain.md#L644)
- 公式解读：该式用于描述本章节对应的变分量、离散量或约束/算法步骤，代码中通过下述函数在单元积分、向量更新、约束投影或线搜索阶段执行。
- 公式（按要求渲染）：
\[
...
## 3. Gradient projection based monolithic scheme > 3.4. Subspace minimization > 3.4.2. Conjugate gradient method for the primal approach
