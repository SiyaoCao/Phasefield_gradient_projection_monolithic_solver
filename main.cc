 ...
 *  to solve the phase-field crack problem
 * 1. The phase-field method treats the phasefield irreversibility using the gradient
 *    projection method. During a load step [t_n, t_n+1], let d_n represent the phasefield
 *    at the beginning of the load step (known), then the inequality constraints
 *    d_n <= d_n+1 <= 1.0 are treated as box constraints.
 * 2. Various direct and iterative linear solvers are designed to improve the wall-clock
 *    run time.
...
namespace PhaseField
{
  ...
  class usr_Jacobi_preconditioner : public Subscriptor
  {
  public:
    usr_Jacobi_preconditioner(const BlockSparseMatrix<double> & S);
  ...
  void usr_Jacobi_preconditioner::vmult(BlockVector<double> & dst,
					const BlockVector<double> & src) const
  {
    PreconditionJacobi<BlockSparseMatrix<double>> preconditioner;
    preconditioner.initialize(*m_system_matrix, 1.0);

    preconditioner.vmult(dst, src);
  }

  // LU preconditioner
  class usr_sparseLU_preconditioner : public Subscriptor
  {
  public:
    usr_sparseLU_preconditioner(const SparseDirectUMFPACK & matrix_factorization);
  ...
  : m_matrix_LU(&matrix_factorization)
  {}

  void usr_sparseLU_preconditioner::vmult(BlockVector<double> & dst,
					       const BlockVector<double> & src) const
  {
    (*m_matrix_LU).vmult(dst, src);
  }

  // Incomplete LU preconditioner
  class usr_sparseILU_preconditioner : public Subscriptor
  {
  public:
    usr_sparseILU_preconditioner(const SparseILU<double> & ILU_factorization_disp,
				 const SparseILU<double> & ILU_factorization_phasefield);
  ...
  usr_sparseILU_preconditioner::usr_sparseILU_preconditioner(const SparseILU<double> & ILU_factorization_disp,
							     const SparseILU<double> & ILU_factorization_phasefield)
  : m_ILU_factorization_disp(& ILU_factorization_disp)
  , m_ILU_factorization_phasefield(& ILU_factorization_phasefield)
  {}

  void usr_sparseILU_preconditioner::vmult(BlockVector<double> & dst,
					   const BlockVector<double> & src) const
  {
    std::vector<types::global_dof_index> sizes_per_block(src.n_blocks());
    for (unsigned int i = 0; i < src.n_blocks(); ++i)
      sizes_per_block[i] = src.block(i).size();
    dst.reinit(sizes_per_block);

    (*m_ILU_factorization_disp).vmult(dst.block(0), src.block(0));
    (*m_ILU_factorization_phasefield).vmult(dst.block(1), src.block(1));
  }
    ...
    void Scenario::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Scenario");
      {
        ...
        prm.declare_entry("Preconditioner type for CG",
                          "ILU",
                          Patterns::Selection("None|Jacobi|LU|ILU"),
                          "Type of preconditioner used to solve the linear system");

        prm.declare_entry("CG tolerance",
                          "1.0e-6",
                          Patterns::Double(0.0),
                          "Convergence tolerance of CG iterations");
  ...
  private:
    ...
    // m_active_set_phasefield has 0 (inactive constraint)
    //                          or 1 (active constraint lower bound)
    //                          or 2 (active constraint upper bound)
    // In order to add active set into the VTK output, we have to declare
    // it as double, not int or unsigned int
    Vector<double> m_active_set_phasefield;
    ...
    void get_error_residual_LBFGSB(Errors &error_residual,
    ...
    void solve_nonlinear_timestep_LBFGS_B(BlockVector<double> &solution_delta,
    				          BlockVector<double> & LBFGS_update_refine);

    void calculate_cauchy_point(const BlockSparseMatrix<double> & B0_matrix,
    ...
    void point_projection(BlockVector<double> & solution_delta);
    ...
    std::priority_queue< std::pair<double, unsigned int>,
      ...
      calculate_break_points(const BlockVector<double> & solution_delta,
			     const BlockVector<double> & gradient_g,
			     BlockVector<double> & gradient_d);

    double ebT_x_B0_x_v(const unsigned int b,
			const BlockSparseMatrix<double> & B0_matrix,
			const BlockVector<double> & v);

    void zT_x_vector(const BlockVector<double> & z,
		     const BlockVector<double> & src_vector,
		     BlockVector<double> & target_vector);

    void z_x_vector(const BlockVector<double> & z,
		    const BlockVector<double> & src_vector,
		    BlockVector<double> & target_vector);

    void zT_B0_z(const BlockVector<double> & z,
		     BlockSparseMatrix<double> & B0_matrix);


  }; // class PhaseFieldSplitSolve

  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::zT_B0_z(const BlockVector<double> & z,
					       BlockSparseMatrix<double> & B0_matrix)
  {
    // block 1: displacement
    for (unsigned int i = 0; i < z.block(m_u_dof).size(); ++i)
      {
	if (z.block(m_u_dof)[i] < 0)
	  {
	    for (auto itr = B0_matrix.block(m_u_dof, m_u_dof).begin(i);
		      itr != B0_matrix.block(m_u_dof, m_u_dof).end(i);
		      ++itr)
	      {
		if (itr->column() != itr->row())
		  {
		    B0_matrix.block(m_u_dof, m_u_dof).set(itr->row(),    itr->column(), 0.0);
		    B0_matrix.block(m_u_dof, m_u_dof).set(itr->column(), itr->row(),    0.0);
		  }
	      }
	  }
      } // for (unsigned int i = 0; i < z.block(m_u_dof).size(); ++i)
    ...
    // block 2: phasefield
    for (unsigned int i = 0; i < z.block(m_d_dof).size(); ++i)
      {
	if (z.block(m_d_dof)[i] < 0)
	  {
	    for (auto itr = B0_matrix.block(m_d_dof, m_d_dof).begin(i);
		      itr != B0_matrix.block(m_d_dof, m_d_dof).end(i);
		      ++itr)
	      {
		if (itr->column() != itr->row())
		  {
		    B0_matrix.block(m_d_dof, m_d_dof).set(itr->row(),    itr->column(), 0.0);
		    B0_matrix.block(m_d_dof, m_d_dof).set(itr->column(), itr->row(),    0.0);
		  }
	      }
	  }
      } // for (unsigned int i = 0; i < z.block(m_d_dof).size(); ++i)
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::z_x_vector(const BlockVector<double> & z,
						  const BlockVector<double> & src_vector,
						  BlockVector<double> & target_vector)
  {
    //We assume that the dimensions of all the block matrices are correct
    //block 1: displacement
    unsigned int target_vector_index = 0;
    for (unsigned int i = 0; i < z.block(m_u_dof).size(); ++i)
      {
	if (z.block(m_u_dof)[i] > 0)
	  {
	    target_vector.block(m_u_dof)[i] = src_vector.block(m_u_dof)[target_vector_index];
	    ++target_vector_index;
	  }
	else
	  target_vector.block(m_u_dof)[i] = 0;
      }
	...
	if (z.block(m_d_dof)[i] > 0)
	  {
	    target_vector.block(m_d_dof)[i] = src_vector.block(m_d_dof)[target_vector_index];
	    ++target_vector_index;
	  }
	else
	  target_vector.block(m_d_dof)[i] = 0;
      }
  }

  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::zT_x_vector(const BlockVector<double> & z,
						   const BlockVector<double> & src_vector,
						   BlockVector<double> & target_vector)
  {
    //We assume that the dimensions of all the block matrices are correct
	...
	if (z.block(m_u_dof)[i] > 0)
	  {
	    target_vector.block(m_u_dof)[target_vector_index] = src_vector.block(m_u_dof)[i];
	    ++target_vector_index;
	  }
      }
    ...
    for (unsigned int i = 0; i < z.block(m_d_dof).size(); ++i)
      {
        if (z.block(m_d_dof)[i] > 0)
        {
	  target_vector.block(m_d_dof)[target_vector_index] = src_vector.block(m_d_dof)[i];
	  ++target_vector_index;
        }
      }
  }
  ...
  template <int dim>
  double PhaseFieldMonolithicSolve<dim>::ebT_x_B0_x_v(const unsigned int b,
						      const BlockSparseMatrix<double> & B0_matrix,
						      const BlockVector<double> & v)
  {
    double row_sum = 0.0;
    for (auto itr = B0_matrix.block(m_d_dof, m_d_dof).begin(b);
              itr != B0_matrix.block(m_d_dof, m_d_dof).end(b);
              ++itr)
      {
        row_sum += itr->value() * v.block(m_d_dof)(itr->column());
      }
								 ...
								 const BlockVector<double> & solution_delta)
  {
    ...
    // For displacement DOFs, except essential boundary conditions
    // and hanging-node constraints, there are no box constraints
    ...
    // For phasefield DOFs, there are points with active box constraints
    // and points with inactive box constraints
    const double upper_limit = 1.0;
    Vector<double> solution_phasefield_total(m_solution.block(m_d_dof));
    solution_phasefield_total += solution_delta.block(m_d_dof);
    ...
    for (unsigned int i = 0; i < m_dofs_per_block[m_d_dof]; ++i)
      {
	// phasefield DOFs can still be constrained due to hanging-nodes
  ...
  void PhaseFieldMonolithicSolve<dim>::make_grid_case_10()
  {
    ...
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
    m_logfile << "\t\t\t\tL-shape bending (3D structured)" << std::endl;
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
    ...
    const double thickness = 150.0;
    const unsigned int n_layer = 11;
  ...
  void PhaseFieldMonolithicSolve<dim>::setup_system()
  {
    ...
    m_tangent_matrix.clear();
    {
      ...
      for (unsigned int ii = 0; ii < m_n_components; ++ii)
        for (unsigned int jj = 0; jj < m_n_components; ++jj)
          {
            if (   ((ii < m_d_component) && (jj == m_d_component))
                || ((ii == m_d_component) && (jj < m_d_component)) )
    ...
    m_active_set_phasefield.reinit(m_dofs_per_block[m_d_dof]);
...
*/
  ...
  {
    m_logfile << "\t\t" << "L-BFGS (warning: without phasefield irreversibility)" << std::endl;;
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::print_conv_header_LBFGSB()
  {
    m_logfile << '\t' << "L-BFGS-B" << std::endl;;
    ...
    m_logfile << "\t itr  "
              << " |    LB    UB    LUB    CG-itr    LS-alpha     Energy      Res_Norm    "
              << " Res_u      Res_d    Inc_Norm   "
              << " Inc_u      Inc_d" << std::endl;
...
*/
			 ...
			 BlockVector<double> & solution_delta_cauchy_point)
  {
    ...
    // We need to make sure the solution_delta_cauchy_point satisfies the essential
    // boundary conditions and the hanging-node constraints
    m_constraints.distribute(solution_delta_cauchy_point);
				   ...
				   BlockVector<double> & LBFGS_update_refine)
  {
    ...
    // all the list goes from k-m to k-1
    // the front is the oldest quantity,and the end is
    // newest quantity
    ...
    unsigned int lower_bound_number_old = 0;
    unsigned int upper_bound_number_old = 0;
    unsigned int lowerupper_bound_number_old = 0;

    unsigned int lower_bound_number_new = 0;
    unsigned int upper_bound_number_new = 0;
    unsigned int lowerupper_bound_number_new = 0;
	...
	if (m_parameters.m_output_iteration_history)
	  m_logfile << '\t' << std::setw(4) << LBFGS_iteration << ' '
        ...
        get_error_residual_LBFGSB(m_error_residual,
				  solution_delta);
		...
		m_logfile << std::endl;
            ...
            if (m_parameters.m_relative_residual)
              {
		m_logfile << "\t\tProjected gradient of disp. (relative): "
			  << m_error_residual_norm.m_u << std::endl;

		m_logfile << "\t\tProjected gradient of disp. (absolute): "
			  << m_error_residual_norm.m_u * m_error_residual_0.m_u << std::endl;

		m_logfile << "\t\tProjected gradient of phasefield (relative): "
			  << m_error_residual_norm.m_d << std::endl;

		m_logfile << "\t\tProjected gradient of phasefield (absolute): "
			  << m_error_residual_norm.m_d * m_error_residual_0.m_d << std::endl;
	...
	// In the iteration LBFGS_iteration = 0, only the essential boundary conditions
	// are applied.
	// WHen LBFGS_iteration = 1, it is the first step of LBFGS update, and the
	// s_vector_list and y_vector_list are empty.
	// Since the pair of s and y will only be added to the list if s dot y > tol,
	// it is safer to decide the matrix dimension by the size of the list.
	const unsigned int list_size = s_vector_list.size();
	const auto itr_s_begin    = s_vector_list.begin();
	const auto itr_y_begin    = y_vector_list.begin();
	const auto itr_b0xs_begin = b0xs_vector_list.begin();
	const auto itr_s_dot_y_begin = s_dot_y_list.begin();
	...
	m_active_set_phasefield = 0;
	calculate_cauchy_point(m_tangent_matrix,
			       y_vector_list,
			       b0xs_vector_list,
			       M_matrix,
			       m_system_rhs,
			       solution_delta,
			       solution_delta_cauchy_point);
	...
	BlockVector<double> search_direction(m_dofs_per_block);
	search_direction = 0.0;

	unsigned int cg_iterations = 0;

	double alpha_backtrack = 1.0;
	...
	if (m_parameters.m_type_linear_solver == "CG")
	  {
		...
		if (m_parameters.m_type_preconditioner == "None")
		  {
		    // somehow op_total_inv has to be made const, or the
		    // program will have compliation error
		  ...
		  {
		    usr_Jacobi_preconditioner preconditioner(m_tangent_matrix);

		    const auto op_total_inv = inverse_operator(op_total, cg, preconditioner);
		    op_total_inv.vmult(search_direction, rhs_vector);
		  }
		else if (m_parameters.m_type_preconditioner == "LU")
		  {
		    SparseDirectUMFPACK matrix_factorization;
		    matrix_factorization.initialize(m_tangent_matrix);

		    usr_sparseLU_preconditioner preconditioner(matrix_factorization);
		    const auto op_total_inv = inverse_operator(op_total, cg, preconditioner);
		    op_total_inv.vmult(search_direction, rhs_vector);
		  }
		else if (m_parameters.m_type_preconditioner == "ILU")
		  {
		    SparseILU<double> SparseILU_disp;
		    SparseILU_disp.initialize(m_tangent_matrix.block(0, 0));
		    SparseILU<double> SparseILU_phasefield;
		    SparseILU_phasefield.initialize(m_tangent_matrix.block(1, 1));

		    usr_sparseILU_preconditioner preconditioner(SparseILU_disp,
								SparseILU_phasefield);
	...
	else if (m_parameters.m_type_linear_solver == "Direct")
	  {
	    ...
	    //zT_B0_z_inv_phasefield.vmult(search_direction.block(m_d_dof), rhs_vector.block(m_d_dof));

	    BlockVector<double> update_vector(m_dofs_per_block);
	    update_vector = 0;
	    if (list_size > 0)
	      {
		std::list<BlockVector<double>> zT_B0_z_inv_zT_y_list;
		std::list<BlockVector<double>> zT_y_list;
		BlockVector<double> zT_y_vector(m_dofs_per_block);
		BlockVector<double> zT_B0_z_inv_zT_y_vector(m_dofs_per_block);
		for (auto itr = y_vector_list.begin(); itr != y_vector_list.end(); ++itr)
		  {
		    zT_y_vector = (*itr);
		    for (unsigned int i = 0; i < m_dofs_per_block[m_u_dof]; ++i)
		    ...
		    //zT_B0_z_inv_disp.vmult(zT_B0_z_inv_zT_y_vector.block(m_u_dof), zT_y_vector.block(m_u_dof));
		    //zT_B0_z_inv_phasefield.vmult(zT_B0_z_inv_zT_y_vector.block(m_d_dof), zT_y_vector.block(m_d_dof));
		    zT_B0_z_inv.vmult(zT_B0_z_inv_zT_y_vector, zT_y_vector);

		    zT_B0_z_inv_zT_y_list.push_back(zT_B0_z_inv_zT_y_vector);
		  }

		std::list<BlockVector<double>> zT_B0_z_inv_zT_b0xs_list;
		std::list<BlockVector<double>> zT_b0xs_list;
		BlockVector<double> zT_b0xs_vector(m_dofs_per_block);
		BlockVector<double> zT_B0_z_inv_zT_b0xs_vector(m_dofs_per_block);
		for (auto itr = b0xs_vector_list.begin(); itr != b0xs_vector_list.end(); ++itr)
		  {
		    zT_b0xs_vector = (*itr);
		    for (unsigned int i = 0; i < m_dofs_per_block[m_u_dof]; ++i)
		    ...
		    //zT_B0_z_inv_disp.vmult(zT_B0_z_inv_zT_b0xs_vector.block(m_u_dof), zT_b0xs_vector.block(m_u_dof));
		    //zT_B0_z_inv_phasefield.vmult(zT_B0_z_inv_zT_b0xs_vector.block(m_d_dof), zT_b0xs_vector.block(m_d_dof));
		    zT_B0_z_inv.vmult(zT_B0_z_inv_zT_b0xs_vector, zT_b0xs_vector);

		    zT_B0_z_inv_zT_b0xs_list.push_back(zT_B0_z_inv_zT_b0xs_vector);
		  }
	...
	if (m_parameters.m_output_iteration_history)
	    ...
	    m_logfile << "  | "
		      << std::setw(6) << number_active_constraint_lower_bound
		      << std::setw(6) << number_active_constraint_upper_bound
		      << std::setw(6) << number_active_constraint_lowerupper_bound;
            if (m_parameters.m_type_linear_solver == "CG")
              m_logfile << std::setw(8) << cg_iterations;
            else
              m_logfile << std::setw(8) << "---";
            m_logfile << "      "
        	      << std::fixed << std::setprecision(3) << std::scientific << line_search_parameter
		      << std::fixed << std::setprecision(6) << std::setw(1) << std::scientific
		      << "  " << energy_functional
		      << std::fixed << std::setprecision(3) << std::setw(1)
					<< std::scientific
		      << "  " << m_error_residual_norm.m_norm
    ...
    AssertThrow(LBFGS_iteration < m_parameters.m_max_iterations_BFGS,
                ExcMessage("No convergence in L-BFGS-B nonlinear solver!"));
  }
  ...
  void PhaseFieldMonolithicSolve<dim>::output_results() const
  {
    ...
    // output phasefield active set status
    BlockVector<double> active_set_status(m_dofs_per_block);
    active_set_status.block(m_d_dof) = m_active_set_phasefield;
    std::vector<DataComponentInterpretation::DataComponentInterpretation>
      data_component_interpretation_active_set(
        dim+1, DataComponentInterpretation::component_is_scalar);
    std::vector<std::string> solution_name_active_set;
    solution_name_active_set.emplace_back("disp_x_active_set");
    solution_name_active_set.emplace_back("disp_y_active_set");
    if (dim ==3)
      solution_name_active_set.emplace_back("disp_z_active_set");
    solution_name_active_set.emplace_back("phasefield_active_set");
    data_out.add_data_vector(active_set_status,
			     solution_name_active_set,
			     DataOut<dim>::type_dof_data,
			     data_component_interpretation_active_set);
	...
	unsigned int adp_refine_iteration = 0;
	    ...
	    solution_delta = 0.0;
	      ...
	      solve_nonlinear_timestep_LBFGS(solution_delta, LBFGS_update_refine);
            else if (m_parameters.m_type_nonlinear_solver == "LBFGSB")
              solve_nonlinear_timestep_LBFGS_B(solution_delta, LBFGS_update_refine);
	    else
...
int main(int argc, char* argv[])
{
  ...
  if (dim == 2 )
    {
      PhaseField::PhaseFieldMonolithicSolve<2> problem_2D("parameters.prm");
      problem_2D.run();
    }
  else if (dim == 3)
    {
      PhaseField::PhaseFieldMonolithicSolve<3> problem_3D("parameters.prm");
      problem_3D.run();
    }
  else
    {
